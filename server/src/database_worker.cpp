...
void DatabaseWorker::processQueue() {
    ...
    while (!g_NeedStop) {
        ...
        if (task.cmd == common::cmd_t::IDLE) {
            ...
            std::string encrypted_pass = std::to_string(utils::crc32(task.request.password));
            db::User user = conn->createUser(task.request.user, encrypted_pass, task.storage, task.request.ip_address,
                                             task.request.city);
            if (user.id == 0) {
            ...
            std::string body = apiclient_utils::build_api_ok_response_body("location", locations);
            logd5(body);
            task.client->sendOkResponse(body);
        } else if (task.cmd == common::cmd_t::CHAT_WITH_LOCATION) {
            logd5("get users from location");
            ...
            std::set <std::string> users = conn->getAllUsersFromLocation(task.request.city);

            if (users.empty()) {
                task.client->sendErrorResponse(404, common::ApiStatusCode::ERR_NOT_FOUND,
                                               "no users from " + task.request.city);
                continue;
            }

            std::string body = apiclient_utils::build_api_ok_response_body("user", users);
            logd5(body);
            task.client->sendOkResponse(body);
        }
    }
