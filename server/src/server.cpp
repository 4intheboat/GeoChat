...
Server::Server(int port, int io_thread_pool_size, std::string sert) :
        m_IoPoolSize(io_thread_pool_size),
        m_Sert(sert),
        m_MainIo(std::make_unique<IoThread>(m_Sert)),
...
void Server::run() {
    ...
    for (size_t i = 0; i < m_IoPoolSize; ++i) {
        m_IoThreads.push_back(std::make_unique<IoThread>(m_Sert));
