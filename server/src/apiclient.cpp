...
//                int min_len = libproperty::Options::impl()->get<int>("pass_len");
        ...
        if (command == common::cmd_t::GET_LOCATIONS) {
            std::string fatal_error;
            auto it = find_required_string_param(document, "client_ip", fatal_error);
            if (!fatal_error.empty()) {
                f::loge("[{0}] parse meta: {1}", details.sessid, fatal_error);
                return fatal_error;
            }
            details.remote_address = it->value.GetString();
            return "";
        }
...
void ApiClient::sendOkResponseAndStartIdle() {
    ...
    m_Timer.async_wait(
            boost::bind(&ApiClient::timerWaitTaskResultHandler, shared_from_this(), boost::asio::placeholders::error));
}
