...
namespace input {
    ...
    std::string cmd2string(cmd_t cmd) {
        switch (cmd) {
            ...
            case cmd_t::LOGIN:
                return "LOGIN";
            case cmd_t::REGISTER_USER:
                return "REGISTER";
            case cmd_t::REGISTER_GROUP:
                return "REGISTER_G";
            case cmd_t::CHAT_WITH_USER:
                return "CHAT_WITH";
            case cmd_t::ADD_TO_GROUP:
                return "ADD_TO_G";
            case cmd_t::HISTORY_USER:
                return "HISTORY";
            case cmd_t::HISTORY_GROUP:
                return "HISTORY_G";
            case cmd_t::DIRECT_MSG_USER:
                return "DIRECT_MSG";
            case cmd_t::DIRECT_MSG_GROUP:
                return "DIRECT_MSG_G";
            case cmd_t::STATUS_USER:
                return "STATUS";
            case cmd_t::STATUS_GROUP:
                return "STATUS_G";
            case cmd_t::QUIT:
                return "QUIT";
            case cmd_t::NONE:
                return "none";
            case cmd_t::GET_LOCATIONS:
                return "GET_LOCATIONS";
            case cmd_t::CHAT_WITH_LOCATION:
                return "CHAT_WITH_LOCATION";
        }
        return "";
    }
        ...
        if (it != document.MemberEnd()) {
            ...
            if (users.IsArray()) {
                for (auto i = 0; i < users.Size(); i++) {
                    if (users[i].IsObject()) {
                        auto it = users[i].FindMember("user");
                        if (it != users[i].MemberEnd()) {
                            response.users.push_back(it->value.GetString());
                        }
                    }
